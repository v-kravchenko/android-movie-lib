...
interface PrimaryTranslationDao {
    ...
    @Delete
    suspend fun delete(primaryTranslation: PrimaryTranslationEntity)

    @Query("DELETE FROM primary_translations")
    suspend fun deleteAll()

    @Update
    suspend fun update(primaryTranslation: PrimaryTranslationEntity)

    @Query("SELECT * FROM primary_translations")
    fun all(): Flow<List<PrimaryTranslationEntity>>

    @Query("SELECT COUNT(1) FROM primary_translations")
