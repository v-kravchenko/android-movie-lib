...
class HomeFragment : BaseFragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    (binding.spinnerLanguage.editText as? AutoCompleteTextView)?.let {
      ...
      it.setOnItemClickListener { _, _, position, _ ->
        languageAdapter.getItem(position)?.let { language -> viewModel.setApiLanguage(language) }
      }
    }
  ...
  private fun setupObserver() {
    ...
    observe(viewModel.loadDictionaryEvent) {
      it.onFailure { throwable -> Timber.d("Error loading config: ${throwable.localizedMessage}") }
    }
  }
}
